...
import {sttStore, ttsStore, uiStore} from './src/store';
...
const App = observer(() => {
  ...
  // Initialize speech stores (persistent settings + foreground lifecycle).
  // Both are idempotent; model downloads remain an explicit user action.
  React.useEffect(() => {
    ttsStore.init().catch(() => {
      // init() swallows its own errors; catch to satisfy no-floating-promises.
    });
    sttStore.init().catch(() => {
      // STT availability checks must never block app startup.
    });
  }, []);
