...
const PAL_PIP: OnboardingPalDef = {
  ...
  name: 'Pip',
  description:
    'A friendly general-purpose agent that runs entirely on your phone.',
  systemPrompt:
  ...
  models: [
    palEntry({
      ...
      filename: 'Huihui-Qwen3.5-0.8B-abliterated.Q8_0.gguf',
      params: 800000000,
    ...
    palEntry({
      ...
      sizeBytes: 3866957760,
    }),
    palEntry({
      tier: 'best',
      recommended: false,
      repo: 'HauhauCS/Gemma-4-E4B-Uncensored-HauhauCS-Aggressive',
      filename: 'Gemma-4-E4B-Uncensored-HauhauCS-Aggressive-Q6_K_P.gguf',
      params: 4000000000,
      displayName: 'Gemma 4 E4B',
      sizeBytes: 6249794528,
    }),
  ],
};
...
const PAL_CODIE: OnboardingPalDef = {
  ...
  systemPrompt:
    "You are Codie, a coding agent on the user's phone. Answer with code first: one fenced block with a language tag. Then explain in at most 2 short sentences — phone screens are small. When debugging, state the bug in one sentence, then show the fixed code. Never repeat the user's code back.",
  color: ['#0F3D5E', '#7BB9D7'],
  greeting: {
