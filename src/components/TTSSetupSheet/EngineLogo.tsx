...
// Partial: engines without a shipped logo asset (inflect) fall through to a
// monogram badge rendered in `renderInner`.
const LOGO_SOURCES: Partial<Record<Exclude<EngineId, 'system'>, number>> = {
...
export const EngineLogo: React.FC<EngineLogoProps> = ({
  ...
  const renderInner = () => {
    ...
    // Inflect has no shipped logo asset yet — render a monogram badge.
    const logo = LOGO_SOURCES[engineId];
    if (!logo) {
      return (
        <View
          style={[
