...
// ---------------------------------------------------------------------------
// Inflect-Nano-v2 (VITS, 4M params, ~16 MB, English, single fixed voice)
// ---------------------------------------------------------------------------

/**
 * HuggingFace base URL for the official Inflect-Nano-v2 ONNX package
 * (duration predictor + HiFi-GAN decoder, dynamic-length graphs).
 */
export const INFLECT_MODEL_BASE_URL =
  'https://huggingface.co/owensong/Inflect-Nano-v2-ONNX/resolve/main';

/** Subdirectory (relative to app documents dir) for Inflect model files. */
export const INFLECT_MODEL_SUBDIR = 'tts/inflect';

/**
 * Inflect's two ONNX graphs. The IPA dict is downloaded separately via
 * `TTS_DICT_URL` into the same directory (shared data source with the JS
 * phonemizer used by Kokoro/Kitten).
 */
export const INFLECT_MODEL_FILES = [
  {name: 'duration.onnx', urlPath: 'onnx/duration.onnx'},
  {name: 'decode.onnx', urlPath: 'onnx/decode.onnx'},
] as const;

/** Output audio sample rate (Hz). Inflect is a 24 kHz model. */
export const INFLECT_SAMPLE_RATE = 24_000;

/** Estimated total size of the Inflect bundle (~16 MB + dict). */
export const INFLECT_MODEL_ESTIMATED_BYTES = 16 * 1024 * 1024;
