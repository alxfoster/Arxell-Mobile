...
export const Onboarding6Screen: React.FC = observer(() => {
  const {l10n, goBack, confirmAndContinue, isFinishing} =
    useOnboardingHandlers(6);
  ...
  const selectedModelName = pickedEntry?.displayName ?? pal.name;
  const primaryLabel = pickedDownloaded
    ? t.screen6.useTemplate.replace('{{name}}', selectedModelName)
    : sizeLabel
      ? t.screen6.ctaTemplate
          .replace('{{name}}', selectedModelName)
          .replace('{{size}}', sizeLabel)
      : t.screen6.cta.replace('{{name}}', selectedModelName);
