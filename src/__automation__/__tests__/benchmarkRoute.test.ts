...
describe('benchmarkRoute', () => {
  describe('isBenchmarkRunnerUrl', () => {
    it('matches the bare bench URL', () => {
      expect(isBenchmarkRunnerUrl('arxell://e2e/benchmark')).toBe(true);
    });

    it('matches the bench URL with a query string (prefix tolerates query)', () => {
      expect(isBenchmarkRunnerUrl('arxell://e2e/benchmark?autostart=1')).toBe(
        true,
      );
    });

    it('does not match unrelated URLs', () => {
      expect(isBenchmarkRunnerUrl('arxell://chat?palId=foo')).toBe(false);
      expect(isBenchmarkRunnerUrl('arxell://e2e/other')).toBe(false);
    });
  ...
  describe('parseBenchmarkAutostart', () => {
    it('returns true for autostart=1', () => {
      expect(
        parseBenchmarkAutostart('arxell://e2e/benchmark?autostart=1'),
      ).toBe(true);
    });

    it('returns true for autostart=true (case-insensitive)', () => {
      expect(
        parseBenchmarkAutostart('arxell://e2e/benchmark?autostart=true'),
      ).toBe(true);
      expect(
        parseBenchmarkAutostart('arxell://e2e/benchmark?autostart=TRUE'),
      ).toBe(true);
    ...
    it('returns true when autostart sits alongside other query params', () => {
      expect(
        parseBenchmarkAutostart('arxell://e2e/benchmark?foo=bar&autostart=1'),
      ).toBe(true);
    });

    it('returns false for autostart=0', () => {
      expect(
        parseBenchmarkAutostart('arxell://e2e/benchmark?autostart=0'),
      ).toBe(false);
    });

    it('returns false for autostart=false', () => {
      expect(
        parseBenchmarkAutostart('arxell://e2e/benchmark?autostart=false'),
      ).toBe(false);
    });

    it('returns false for an unrecognized value (garbage)', () => {
      expect(
        parseBenchmarkAutostart('arxell://e2e/benchmark?autostart=banana'),
      ).toBe(false);
      expect(parseBenchmarkAutostart('arxell://e2e/benchmark?autostart=')).toBe(
        false,
      );
      expect(
        parseBenchmarkAutostart('arxell://e2e/benchmark?autostart=11'),
      ).toBe(false);
    });

    it('returns false for the bare URL (no query)', () => {
      expect(parseBenchmarkAutostart('arxell://e2e/benchmark')).toBe(false);
    });

    it('returns false when the query has no autostart key', () => {
      expect(parseBenchmarkAutostart('arxell://e2e/benchmark?foo=bar')).toBe(
        false,
      );
    });
    ...
    it('iOS-origin and Android-origin parity: same raw URL resolves identically', () => {
      ...
      const url = 'arxell://e2e/benchmark?autostart=1';
